...
const replicationState = myCollection.syncGraphQL({
    ...
    pull: {
        ...
        modifier: doc => doc // (optional) modifies all pulled documents before they are handeled by RxDB. Can be an async function.
    },
...
const replicationState = myCollection.syncGraphQL({
    ...
    push: {
        ...
        modifier: d => d // (optional) modifies all pushed documents before they are send to the GraphQL endpoint. Can be an async function.
    },
    deletedFlag: 'deleted', // the flag which indicates if a pulled document is deleted
